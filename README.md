...
### v5.0 — 2026-07-01 · 策略精簡與四均線升級 (4-EMA)

- **策略轉換**：側邊欄策略模式由「三均線 + 布林通道 (EMA/BB)」升級為「四均線 (EMA 20/50/120/240)」，對齊後端升級。
- **布林通道完全移除**：從前端卡片（個別標的分析卡）與 Modal 詳情彈窗中徹底移除所有布林通道顯示（如上下軌、帶寬、%B 值與 Squeeze 狀態）。
- **四均線欄位顯示**：前端卡片佈局與表格展示重構為 EMA 20、50、120、240 四個主要週期，提供最純粹的均線趨勢與交叉/回測訊號資訊。
